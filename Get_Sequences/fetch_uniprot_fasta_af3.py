...
def main():
    ...
    for index, row in df.iterrows():
        ...
        line = {
        'nb_residues_a': len(seq1),
        'nb_residues_b': len(seq2),
        'nb_residues_pdb_a_before_align': len(seq1_pdb),
        'nb_residues_pdb_b_before_align': len(seq2_pdb),
        'nb_residues_uniprot_a_before_align': len(seq1_uniprot),
        'nb_residues_uniprot_b_before_align': len(seq2_uniprot),
        'pdb_id': pdb_id,
        'uniprot_id_a': uniprot_ids[0],
        'uniprot_id_b': uniprot_ids[1],
        'chain_a': chain_ids[0],
        'chain_b': chain_ids[1],
        }
        data.append(line)
        ...
        if seq1 and seq2:
            ...
            json_filename = os.path.join(output_dir, f"{row['id']}.json")
    ...
    # Convert list to DataFrame or Series
    df = pd.DataFrame(data)

    # Save to CSV
    df.to_csv('nb_residues.csv', index=False)
