...
def get_trained_model(config, model_loc=None, model_name=None, override_custom=False):
    if model_name is not None:
        model_loc = os.path.join(config.platform.thermompnn_dir, 'models/')
        model_loc = os.path.join(model_loc, model_name)
        return TransferModelPL.load_from_checkpoint(model_loc, cfg=config).model
    else:
        ckpt_files = glob(os.path.join(model_loc, "*.ckpt"))[0]
        ckpt_files = os.path.basename(ckpt_files)
        if override_custom:
            ...
            model_loc = os.path.join(model_loc, ckpt_files)
            return TransferModelPL.load_from_checkpoint(model_loc, cfg=config).model
...
def run_prediction_default(name, model, dataset_name, dataset, results):
    ...
    for i, batch in enumerate(tqdm(dataset)):
        pdb, mutations, disto, bin_to_sum = batch
        pred, _ = model(pdb, mutations, disto, bin_to_sum)
...
def main(cfg, args):
    ...
    models = {
        ...
        "ThermoMPNN": get_trained_model(config=cfg, model_loc=args.model_path, override_custom=False),
...
#        #"SSYM_dir": ddgBenchDataset(cfg, pdb_dir=os.path.join(misc_data_loc, 'protddg-bench-master/SSYM/pdbs'),
    ...
    df.to_csv(os.path.join(args.model_path, "ThermoMPNN_metrics.csv"))
...
if __name__ == "__main__":
    ...
    cfg = OmegaConf.load("local_test.yaml")
    with torch.no_grad():
        main(cfg, args)
