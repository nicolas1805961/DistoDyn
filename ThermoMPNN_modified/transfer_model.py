...
def get_protein_mpnn(cfg, version='v_48_020.pt'):
    ...
    print(f"Loading ProteinMPNN weights from {checkpoint_path}")
    ...
    model = ProteinMPNN(ca_only=False, num_letters=21, node_features=hidden_dim, edge_features=hidden_dim, hidden_dim=hidden_dim, use_disto_features=cfg.training.use_distogram_features, mode=cfg.training.mode, top_k_other=cfg.training.top_k_other,
                        num_encoder_layers=num_layers, num_decoder_layers=num_layers, k_neighbors=top_k, augment_eps=0.0)
    if cfg.model.load_pretrained:
        model_state = model.state_dict()
        ckpt_state = checkpoint["model_state_dict"]

        key = "features.edge_embedding.weight"
        if ckpt_state[key].shape != model_state[key].shape:
            old_w = ckpt_state[key]
            new_w = model_state[key]

            assert new_w.shape[1] > old_w.shape[1], \
                "New edge features must be appended, not reordered."
            
            print(f"ProteinMPNN edge features expanded: "f"{old_w.shape[1]} → {new_w.shape[1]}")
            
            # copy pretrained weights
            new_w[:, :old_w.shape[1]] = old_w

            # initialize distogram features
            torch.nn.init.xavier_uniform_(new_w[:, old_w.shape[1]:])

            # overwrite model state
            model_state[key] = new_w

            # remove key so load_state_dict won't complain
            del ckpt_state[key]
...
class TransferModel(nn.Module):
    ...
    def forward(self, pdb, mutations, disto, bins_to_sum, tied_feat=True):        
        ...
        all_mpnn_hid, mpnn_embed, _ = self.prot_mpnn(X, S, mask, chain_M, residue_idx, chain_encoding_all, disto, bins_to_sum, None)
