        ...
        for record, coord, pad_mask in zip(records, coords, pad_masks):
            ...
            for model_idx in range(coord.shape[0]):
                ...
                if "logits" in prediction:
                    ...
                    distogram_dict = {'distogram': {'logits': softmax, 'bin_edges': bin_edges, 'prob_contact': prob_contact}}
                    path = (
                        struct_dir
                        / f"distogram_{record.id}_model_{idx_to_rank[model_idx]}.pkl"
                    )
                    with open(path, 'wb') as f:
                        pickle.dump(distogram_dict, f)
