...
def cif_to_graph(distance_file, correlation_file, fasta_file, d_threshold):
    ...
    with open(distance_file, "rb") as f1:
        data = pickle.load(f1)
    ...
    with open(correlation_file, "rb") as f2:
        data = pickle.load(f2)
        dmat_correlation = torch.from_numpy(data['matrix'])
        assert len(dmat_correlation) == len(fasta_sequence), f"correlation matrix size {dmat_correlation.shape} does not match sequence length {len(fasta_sequence)}"

    mask_distance = dmat_distance < d_threshold
    ...
    mask_corr = torch.abs(dmat_correlation) > 0.3
    edge_index_corr = torch.nonzero(mask_corr, as_tuple=False).T
    corr = dmat_correlation[edge_index_corr[0], edge_index_corr[1]]
    corr = corr[:, None]  # (E, 1)
...
def build_graph_dataset(base_distance_folder, base_correlation_folder, output_base_folder, base_fasta_folder, test_lines, val_lines, train_lines):
    """
    ...
    correlation_files = glob(os.path.join(base_correlation_folder, "*.pkl"))
    if not correlation_files:
        print(f"No PKL files found in {base_correlation_folder}")
        return
    ...
    filtered_correlation_files = [f for f in correlation_files if os.path.splitext(os.path.basename(f))[0] in valid_ids]

    print(f"Kept {len(filtered_fasta_files)} FASTA files (out of {len(fasta_files)})")
    print(f"Kept {len(filtered_correlation_files)} correlation files (out of {len(correlation_files)})")
    ...
    correlation_files = sorted(filtered_correlation_files, key=lambda f: os.path.basename(f))  # Sort files by name for consistency
    fasta_files = sorted(filtered_fasta_files, key=lambda f: os.path.basename(f))  # Sort files by name for consistency
    
    assert len(distance_files) == len(correlation_files), f"Number of distance files ({len(distance_files)}) does not match number of correlation files ({len(correlation_files)})"
    
    for distance_file, correlation_file, fasta_file in tqdm(zip(distance_files, correlation_files, fasta_files), total=len(distance_files), desc="Processing files"):
        ...
        assert os.path.basename(correlation_file) == os.path.basename(fasta_file).replace('.fasta', '.pkl'), f"File names do not match: {correlation_file}, {fasta_file}"

        print(f"Processing: {os.path.basename(distance_file)}")
        #print(cif_file)
        #print(fasta_file)
        #print(f"Saved: {save_path}")
        graph = cif_to_graph(distance_file, correlation_file, fasta_file, d_threshold=10)  # <-- your function to parse CIF
...
#
    ...
    base_correlation_folder = '/pasteur/appa/scratch/nportal/MISATO/correlations'
    output_base_folder = '/pasteur/appa/scratch/nportal/MISATO/boltz_parsed/Binding_site/pt_folder_distances_correlation'
    ...
    build_graph_dataset(
        base_distance_folder, base_correlation_folder, output_base_folder, base_fasta_folder, test_lines, val_lines, train_lines
    )
