...
from model import RGCNNodeClassifier, RGATNodeClassifier, Custom_EGNN, RGAT_bs, RGCN_bs, REGNNNodeClassifier, RGCN_bs_paper  # The GNN model we defined
...
class Trainer:
    def __init__(self, config):
        ...
        if self.config["training"]["loss"] == "CrossEntropyLoss":
            ...
            class_weight = torch.tensor([8.29823171917851], dtype=torch.float).to(device)
    ...
    def train_loop_RGAT(self, model, loader, optimizer, device, writer, epoch):
        ...
        for batch in loader_iter:
            ...
            #    loss = self.compute_loss_ce(pred, batch.y, writer, epoch)
            #else:
            #    loss = self.compute_loss_bce(pred, batch.y, writer, epoch)
            loss.backward()
            optimizer.step()
