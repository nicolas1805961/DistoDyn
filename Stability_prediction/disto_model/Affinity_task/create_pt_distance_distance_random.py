...
def cif_to_graph(pkl_file, d_threshold):
    ...
    # dmat: (N, N)
    N = dmat.size(0)
    num_random_edges = dists.size(0)  # choose how many random edges you want

    # Sample random (i, j) pairs
    i = torch.randint(0, N, (num_random_edges,), device=dmat.device)
    j = torch.randint(0, N, (num_random_edges,), device=dmat.device)

    edge_index_random = torch.stack([i, j], dim=0)  # (2, E_random)

    # Edge features from dmat
    dists_random = dmat[i, j]
    dists_random = 1.0 - dists_random / d_threshold
    dists_random = dists_random[:, None]  # (E_random, 1)

    edge_type_random = torch.ones(num_random_edges, dtype=torch.long)
...
def build_graph_dataset(base_folder, output_base_folder):
    """
    ...
    for pkl_file in tqdm(pkl_files, total=len(pkl_files), desc="Processing files"):
        ...
        graph = cif_to_graph(pkl_file, d_threshold=4.5)  # <-- your function to parse CIF
...
#
    ...
    output_base_folder = '/pasteur/appa/scratch/nportal/MISATO/Affinity/pt_folder_distance_distance_random'
