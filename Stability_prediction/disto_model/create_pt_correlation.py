...
def cif_to_graph(correlation_file, fasta_file):
    ...
    with open(correlation_file, "rb") as f:
        ...
        assert len(dmat) == len(fasta_sequence), f"correlation matrix size {dmat.shape} does not match sequence length {len(fasta_sequence)}"

    mask = torch.abs(dmat) > 0.3
    edge_index = torch.nonzero(mask, as_tuple=False).T
    dists = dmat[edge_index[0], edge_index[1]]
    dists = dists[:, None]  # (E, 1)

    #fig, ax = plt.subplots(1, 1)
    #ax.imshow(mask, cmap='grey', interpolation='none')
    #plt.show()

    # Single relation: correlation
...
def build_graph_dataset(base_correlation_folder, output_base_folder, base_fasta_folder, test_lines, val_lines, train_lines):
    """
    ...
    distance_ids = {os.path.splitext(os.path.basename(f))[0] for f in correlation_files}
    ...
    correlation_files = sorted(correlation_files, key=lambda f: os.path.basename(f))  # Sort files by name for consistency
    fasta_files = sorted(filtered_fasta_files, key=lambda f: os.path.basename(f))  # Sort files by name for 
    
    for correlation_file, fasta_file in tqdm(zip(correlation_files, fasta_files), total=len(correlation_files), desc="Processing files"):
        ...
        print(f"Processing: {os.path.basename(correlation_file)}")

        graph = cif_to_graph(correlation_file, fasta_file)  # <-- your function to parse CIF
        ...
        name = os.path.splitext(os.path.basename(correlation_file))[0]  # remove .pkl
...
#
    ...
    output_base_folder = '/pasteur/appa/scratch/nportal/MISATO/Binding_site/pt_folder_correlations'
    ...
    build_graph_dataset(
        base_correlation_folder, output_base_folder, base_fasta_folder, test_lines, val_lines, train_lines
    )
