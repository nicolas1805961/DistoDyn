...
MAX_QUEUED=4          # Max jobs allowed in queue for your user
BASE_DIR="/pasteur/appa/scratch/nportal/boltz/stability_prediction/results_mutated/"
#BASE_DIR="data/Sequences/temp"

echo "Creating chunk lists from $BASE_DIR..."
ls -1 "$BASE_DIR" | split -l $CHUNK_SIZE -d - split_list/split_list_mutated_

chunk_files=(split_list/split_list_mutated_*)
...
for LIST in "${chunk_files[@]}"; do
    ...
    job_output=$(sbatch --exclude=maestro-3478 --array=1-${NUM_LINES} --export=LIST_FILE=$LIST split.sh)
    echo "$job_output"
