...
from model import RGAT_affinity, RGCN_affinity, New_RGCN_complex_affinity, RGCN_affinity_bn, RGCN_affinity_no_residual, RGAT_affinity_no_residual, RGAT_complex_affinity, REGNN_complex_affinity  # The GNN model we defined
...
class Trainer:
    ...
    def compute_loss(self, pred, target, writer, epoch):
        loss = self.loss_fn(pred, target)  # CrossEntropyLoss expects Long targets
    ...
    def train_loop_EGNN(self, model, loader, optimizer, device, writer, epoch):
        model.train()
        total_loss = 0
        loader_iter = tqdm(loader, desc="Training", leave=False)
        for batch in loader_iter:
    ...
    def eval_loop_EGNN(self, model, loader, device, writer, epoch):
        model.eval()
        total_loss = 0
        loader_iter = tqdm(loader, desc="Evaluating", leave=False)
        with torch.no_grad():
    ...
    def train_loop_RGCN(self, model, loader, optimizer, device, writer, epoch):
        model.train()
        total_loss = 0
        loader_iter = tqdm(loader, desc="Training", leave=False)
        for batch in loader_iter:
    ...
    def eval_loop_RGCN(self, model, loader, device, writer, epoch):
        model.eval()
        total_loss = 0
        loader_iter = tqdm(loader, desc="Evaluating", leave=False)
        with torch.no_grad():
...
def main():
    ...
    train_dataset = ProteinGraphDataset(packed_file=os.path.join(data_path, "train", "train_all_graphs.pt"),)
    val_dataset = ProteinGraphDataset(packed_file=os.path.join(data_path, "val", "val_all_graphs.pt"),)
