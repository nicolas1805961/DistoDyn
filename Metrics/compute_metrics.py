...
def compute_metrics(pdb1_path, pdb2_path):
    ...
    return result.tm_norm_chain1, result.tm_norm_chain2, result.rmsd
...
def compare_pdb_folders(gt_folder, pred_folder, output_file="tm_results.csv"):
    ...
    print(f"{'File':<30} {'RMSD':>10} {'TM_gt':>10} {'TM_pred':>10}")
    print("-" * 60)
    ...
    for fname in common_files:
        ...
        try:
            tm_gt, tm_pred, rmsd = compute_metrics(gt_path, pred_path)
            print(f"{fname:<30} {rmsd:10.3f} {tm_gt:10.3f} {tm_pred:10.3f}")
            results.append((fname, rmsd, tm_gt, tm_pred))
        except Exception as e:
            print(f"{fname:<30} ERROR: {e}")
            results.append((fname, "ERROR", "ERROR", "ERROR"))

    with open(output_file, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Filename", "RMSD", "TM_score_gt", "TM_score_pred"])
        for row in results:
            writer.writerow(row)
