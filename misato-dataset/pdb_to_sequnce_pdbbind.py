...
h5_to_pdb_sequences, ca_oords = parse_sequences_from_pdb(pdb_file_path)
...
for seq_idx, seq in enumerate(h5_to_pdb_sequences):
    ...
    print(f"Processing sequence {seq_idx} with length {len(seq)}")

    if seq_idx == len(h5_to_pdb_sequences) - 1:
        continue
...
# Output
output_dir_fasta = "./boltz_inputs_fasta"
